...
# Author: Mayank
# Date: 10/11/2024
PROTOCOL_FILE="vpcflowlogparser/protocol_mapper/protocol-numbers-1.csv"
# FILEPATH=f"{os.getcwd()}/{PROTOCOL_FILE}"
class ProtocolMapper(object):
    ...
    """
    def __init__(self):
        self.filepath = f"{os.getcwd()}/{PROTOCOL_FILE}"
        # self.filepath = FILEPATH
        if not os.path.exists(self.filepath):
            raise FileNotFoundError(f"File not found: {self.filepath}")
        if os.stat(self.filepath).st_size == 0:
            raise ValueError(f"{self.filepath} file is empty")
    
    def parse_protocols(self):
        with open(self.filepath, 'r') as protof:
            lines = protof.readlines()
        mapper_dict = {}
        for line in lines:
            l = line.split(",")
            if l[0].isnumeric():
                #you arr here that means line has a protocol mapping
                protocol_decimal = l[0]
                if l[1]:
                    protocol_name = l[1]
                    mapper_dict[int(protocol_decimal)] = protocol_name.lower()
        return mapper_dict

if __name__ == "__main__":
    pmobj = ProtocolMapper()
    print(pmobj.parse_protocols())
